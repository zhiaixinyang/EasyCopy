 ...
 *         copy some code from android.media.MediaFile
 */
public class CopySysMediaFile {
    // Audio file types
    ...
    private static final HashMap<String, MediaFileType> sExtensionToMediaFileTypeMap
            = new HashMap<String, MediaFileType>();
    private static final HashMap<String, Integer> sMimeTypeToFileTypeMap
            = new HashMap<String, Integer>();
    // maps file extension to MTP format code
    private static final HashMap<String, Integer> sExtensionToFormatMap
            = new HashMap<String, Integer>();
    ...
    static void addFileType(String extension, int fileType, String mimeType) {
        sExtensionToMediaFileTypeMap.put(extension, new MediaFileType(fileType, mimeType));
        sMimeTypeToFileTypeMap.put(mimeType, Integer.valueOf(fileType));
    }

    static void addFileType(String extension, int fileType, String mimeType, int mtpFormatCode) {
        addFileType(extension, fileType, mimeType);
        sExtensionToFormatMap.put(extension, Integer.valueOf(mtpFormatCode));
    ...
    /**
     * 通过文件路径获取 MediaFileType
     *
     * @param path
     * @return MediaFileType
     */
    public static MediaFileType getMediaFileTypeForPath(String path) {
        int lastDot = path.lastIndexOf('.');
        if (lastDot < 0)
            return null;
        return sExtensionToMediaFileTypeMap.get(path.substring(lastDot + 1).toUpperCase(Locale.ROOT));
    }

    /**
     * 通过 extension 获取 MediaFileType
     *
     * @param extension 类型 如 mp3，如果是 .mp3 或文件路径，也支持
     * @return MediaFileType
     */
    public static MediaFileType getMediaFileTypeForExtension(String extension) {
        int lastDot = extension.lastIndexOf('.');
        if (lastDot >= 0) {
            extension = extension.substring(lastDot + 1).toUpperCase(Locale.ROOT);
        }
        return sExtensionToMediaFileTypeMap.get(extension);
    }

    /**
     * 通过 mimType 获取 MediaFileType
     *
     * @param mimeType
     * @return MediaFileType
     */
    public static MediaFileType getMediaFileTypeForMimeType(String mimeType) {
    ...
    public static String getMimeTypeForFile(String path) {
        MediaFileType mediaFileType = getMediaFileTypeForPath(path);
    ...
    public static int getFormatCode(String fileName, String mimeType) {
        ...
        if (lastDot > 0) {
            ...
            Integer value = sExtensionToFormatMap.get(extension);
            if (value != null) {
                return value.intValue();
            }
        }
